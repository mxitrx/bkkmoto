...
export default function App() {
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    const qrData = encodeURIComponent(`MOTO|${formData.name}|${selectedTicket.name}|${Date.now()}`);
  ...
  const customerCss = `
    ...
    .track-bg {
      ...
      background-image: 
        radial-gradient(circle at 100% 100%, transparent 40%, rgba(255,255,255,0.05) 41%, transparent 42%),
        radial-gradient(circle at 0% 0%, transparent 60%, rgba(255,255,255,0.05) 61%, transparent 62%);
    ...
    .logo { font-size: 30px; font-weight: 900; color: #fff; display: flex; align-items: center; gap: 12px; cursor: pointer; letter-spacing: -1px; text-transform: uppercase; }
    .logo-mark { width: 40px; height: 40px; background: var(--primary); display: flex; align-items: center; justify-content: center; font-size: 20px; font-weight: 900; color: #fff; border-radius: 50%; box-shadow: 0 0 15px var(--primary-glow); }
    ...
    .countdown-wrap { display: flex; gap: 15px; margin-bottom: 40px; }
    .cd-box { background: rgba(0,0,0,0.6); border: 1px solid var(--border); backdrop-filter: blur(8px); width: 85px; height: 85px; display: flex; flex-direction: column; align-items: center; justify-content: center; border-radius: 24px; box-shadow: 0 10px 20px rgba(0,0,0,0.2); }
    .cd-num { font-size: 32px; font-weight: 900; color: #fff; line-height: 1; }
    ...
    .form-input { width: 100%; padding: 18px 24px; background: #0f0f11; border: 1px solid var(--border); color: #fff; font-size: 16px; transition: 0.3s; border-radius: 20px; font-weight: 600; }
      ...
      <>
        ...
        <nav className={`navbar ${isScrolled ? 'scrolled' : ''}`}>
          <div className="container nav-wrap">
            <a onClick={() => scrollTo('home')} className="logo"><div className="logo-mark">M</div> <div>{config.title?.split(' ')[0]}<span style={{fontWeight:300, color: 'var(--primary)'}}> {config.title?.split(' ')[1] || 'GP'}</span></div></a>
            ...
            <button className="btn btn-primary" style={{padding: '12px 28px', fontSize: '14px'}} onClick={() => scrollTo('register')}>GET TICKETS</button>
          </div>
        </nav>
        ...
        {/* Stats Banner */}
        <div style={{ background: '#0f0f11', padding: '60px 0', borderBottom: '1px solid var(--border)' }}>
          <div className="container grid-4" style={{ textAlign: 'center' }}>
            <div><div style={{ fontSize: '50px', fontWeight: 900, color: '#fff' }}>350<span style={{fontSize:'20px', color:'var(--primary)'}}>KM/H</span></div><div style={{ color: 'var(--text-muted)', fontSize: '13px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '2px' }}>Top Speed</div></div>
            <div><div style={{ fontSize: '50px', fontWeight: 900, color: '#fff' }}>4.5<span style={{fontSize:'20px', color:'var(--primary)'}}>KM</span></div><div style={{ color: 'var(--text-muted)', fontSize: '13px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '2px' }}>Track Length</div></div>
            <div><div style={{ fontSize: '50px', fontWeight: 900, color: '#fff' }}>12</div><div style={{ color: 'var(--text-muted)', fontSize: '13px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '2px' }}>Turns</div></div>
            <div><div style={{ fontSize: '50px', fontWeight: 900, color: '#fff' }}>25</div><div style={{ color: 'var(--text-muted)', fontSize: '13px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '2px' }}>Laps</div></div>
          </div>
        </div>
          ...
          <section id="video" className="section section-alt">
            <div className="container" style={{ maxWidth: '1000px' }}>
              <div className="sec-header reveal">
                ...
                <p style={{ color: '#a1a1aa', marginTop: '10px', fontSize: '16px' }}>{config.videoDesc}</p>
              </div>
        ...
        <section id="concept" className="section">
          <div className="container">
            ...
            <div className="grid-4 reveal delay-1">
              <div className="concept-card"><div className="concept-icon">🏍️</div><h3 style={{ fontSize: '20px', fontWeight: 900, color: '#fff', marginBottom: '10px', textTransform: 'uppercase' }}>Superbikes</h3><p style={{ fontSize: '14px', color: '#a1a1aa' }}>สัมผัสรถแข่ง 1000cc ตัวจริงที่ใช้แข่งขันในรายการชิงแชมป์โลก</p></div>
              <div className="concept-card delay-1"><div className="concept-icon">🔧</div><h3 style={{ fontSize: '20px', fontWeight: 900, color: '#fff', marginBottom: '10px', textTransform: 'uppercase' }}>Pit Garage</h3><p style={{ fontSize: '14px', color: '#a1a1aa' }}>ชมการทำงานของทีมช่างระดับพระกาฬในการเซ็ตอัพรถก่อนลงสนาม</p></div>
              <div className="concept-card delay-2"><div className="concept-icon">🎧</div><h3 style={{ fontSize: '20px', fontWeight: 900, color: '#fff', marginBottom: '10px', textTransform: 'uppercase' }}>Fan Festival</h3><p style={{ fontSize: '14px', color: '#a1a1aa' }}>โซนกิจกรรมสำหรับไบค์เกอร์ คอนเสิร์ต และการออกบูธจากแบรนด์ชั้นนำ</p></div>
              <div className="concept-card delay-3"><div className="concept-icon">🏁</div><h3 style={{ fontSize: '20px', fontWeight: 900, color: '#fff', marginBottom: '10px', textTransform: 'uppercase' }}>Grid Walk</h3><p style={{ fontSize: '14px', color: '#a1a1aa' }}>โอกาสเดียวที่จะได้เดินบนจุดสตาร์ทกระทบไหล่นักบิดก่อนไฟแดงดับลง</p></div>
            </div>
          </div>
        </section>
              ...
              {config.speakers?.map((speaker, i) => (
                <div key={speaker.id} className={`speaker-card reveal delay-${i%4}`} onClick={() => setSelectedSpeaker(speaker)}>
                  ...
                  <div className="speaker-info">
                    <h3 style={{ fontSize: '22px', fontWeight: 900, color: '#fff', marginBottom: '5px', textTransform: 'uppercase' }}>{speaker.name}</h3>
                    <p style={{ fontSize: '13px', color: 'var(--primary)', fontWeight: 800, letterSpacing: '1px', textTransform: 'uppercase' }}>{speaker.role}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="timeline-wrap reveal delay-1">
              {config.schedule?.map((s) => (
                <div key={s.id} className="time-card">
                  ...
                  <div>
                    <div style={{ display: 'inline-block', fontSize: '11px', fontWeight: '900', color: '#fff', background: s.color, padding: '4px 12px', marginBottom: '12px', letterSpacing: '1px', textTransform: 'uppercase', borderRadius: '50px' }}>{s.tag}</div>
                    <h3 style={{ fontSize: '24px', fontWeight: 900, color: '#fff', marginBottom: '8px', textTransform: 'uppercase' }}>{s.title}</h3>
                    <p style={{ fontSize: '15px', color: '#a1a1aa', lineHeight: 1.6, fontWeight: 500 }}>{s.desc}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Marquee Sponsors */}
        <div style={{ backgroundImage: `url(${config.sponsorBg})`, backgroundSize: 'cover', backgroundAttachment: 'fixed', position: 'relative', padding: '120px 0', overflow: 'hidden', borderTop: '1px solid rgba(255,255,255,0.1)', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
          <div style={{ position: 'absolute', inset: 0, background: 'rgba(15, 15, 17, 0.90)' }}></div>
          <div className="container text-center reveal" style={{ position: 'relative', zIndex: 1, marginBottom: '60px' }}>
            <h3 style={{ color: '#fff', fontSize: '32px', fontWeight: 900, letterSpacing: '4px', textTransform: 'uppercase' }}>OFFICIAL PARTNERS</h3>
          ...
          <div style={{ position: 'relative', zIndex: 1, display: 'flex', overflow: 'hidden', whiteSpace: 'nowrap' }}>
            <div style={{ display: 'flex', animation: 'marquee 25s linear infinite' }}>
              {config.sponsors?.map(s => <div key={s.id} style={{ fontSize: '48px', fontWeight: 900, color: 'transparent', WebkitTextStroke: '1px rgba(255,255,255,0.4)', margin: '0 60px', textTransform: 'uppercase', letterSpacing: '4px' }}>{s.name}</div>)}
              {config.sponsors?.map(s => <div key={s.id+'dup'} style={{ fontSize: '48px', fontWeight: 900, color: 'transparent', WebkitTextStroke: '1px rgba(255,255,255,0.4)', margin: '0 60px', textTransform: 'uppercase', letterSpacing: '4px' }}>{s.name}</div>)}
            </div>
          </div>
        </div>
        ...
        <section id="register" className="section section-alt">
          <div className="container">
            <div className="sec-header reveal">
              ...
              <h2 className="sec-title">จองบัตรเข้าชมการแข่งขัน</h2>
              <div className="sec-line"></div>
            </div>
            ...
            <div className="grid-3 reveal delay-1" style={{ marginBottom: '80px', alignItems: 'end' }}>
              {config.tickets?.map(ticket => (
                <label key={ticket.id} className="ticket-wrapper">
                  ...
                  <div className="ticket-card" style={ticket.type === 'vip' ? { transform: 'scale(1.05)', background: `linear-gradient(180deg, ${config.primaryColor ? config.primaryColor + '1A' : 'rgba(220, 38, 38, 0.1)'} 0%, rgba(24, 24, 27, 1) 100%)`, borderColor: 'var(--primary)' } : {}}>
                    ...
                    <h3 style={{ fontSize: '28px', fontWeight: '900', color: '#fff', textAlign: 'center', textTransform: 'uppercase' }}>{ticket.name}</h3>
                    <div style={{ fontSize: '54px', fontWeight: '900', color: ticket.type === 'vip' ? 'var(--primary)' : '#fff', margin: '15px 0 30px', textAlign: 'center', lineHeight: 1 }}>{Number(ticket.price).toLocaleString()} <span style={{ fontSize: '18px', color: '#a1a1aa', fontWeight: 700, fontStyle: 'normal' }}>THB</span></div>
                    <div style={{ flex: 1, marginBottom: '30px' }}>
                      {ticket.features.split('\n').map((f, i) => (
                        <div key={i} style={{ padding: '12px 0', display: 'flex', gap: '15px', alignItems: 'flex-start', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: '15px', color: '#e4e4e7', fontWeight: 500 }}>
                          <span style={{ color: ticket.type === 'vip' ? 'var(--primary)' : 'var(--secondary)', fontWeight: 900, fontSize: '16px' }}>✓</span> {f}
                        </div>
                      ))}
                    </div>
                    <div className="btn" style={{ width: '100%', background: String(formData.ticketId) === String(ticket.id) ? 'var(--primary)' : 'var(--bg-card)', color: String(formData.ticketId) === String(ticket.id) ? '#fff' : '#fff', border: String(formData.ticketId) === String(ticket.id) ? 'none' : '1px solid var(--border)' }}>
            ...
            <div className="form-box-wrapper reveal">
              <div className="form-box">
                <h3 style={{ fontSize: '28px', fontWeight: 900, marginBottom: '30px', color: '#fff', display: 'flex', alignItems: 'center', gap: '15px', textTransform: 'uppercase' }}>
                  <span style={{ width: '40px', height: '40px', background: 'var(--primary)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', borderRadius: '50%' }}>🏁</span> Checkout
                </h3>
                <form onSubmit={handleRegisterSubmit}>
                  ...
                  <div style={{ background: 'rgba(0,0,0,0.5)', border: '1px solid var(--border)', padding: '40px', marginTop: '40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '30px', borderRadius: '32px' }}>
                    <div>
                      <div style={{ fontSize:'13px', fontWeight: '900', letterSpacing: '1px', textTransform: 'uppercase', color: 'var(--primary)', marginBottom: '8px' }}>🎟️ {selectedTicket?.name || '-'} (1 PASS)</div>
                      <div style={{ color: '#a1a1aa', fontSize: '15px', fontWeight: 500 }}>Subtotal: {subtotal.toLocaleString()} ฿ | VAT 7%: {vat.toLocaleString()} ฿</div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '30px' }}>
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: '11px', color: '#a1a1aa', textTransform: 'uppercase', fontWeight: 800, letterSpacing: '1px' }}>Total Amount</div>
                        <div style={{ fontSize: '36px', fontWeight: '900', color: '#fff', lineHeight: 1 }}>{total.toLocaleString()} <span style={{ fontSize: '18px', fontWeight: 700, fontStyle: 'normal' }}>฿</span></div>
                      </div>
                      <button type="submit" className="btn btn-primary" style={{ padding: '16px 36px', fontSize: '16px' }} disabled={isSubmitting}>
        ...
        <footer style={{ background: '#000', padding: '80px 0 40px', borderTop: '2px solid var(--border)' }}>
          <div className="container" style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '40px', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '40px', marginBottom: '40px' }}>
            <div>
              ...
              <p style={{ fontSize: '15px', color: '#a1a1aa', maxWidth: '350px', lineHeight: 1.6, fontWeight: 500 }}>{config.aboutText.substring(0, 80)}...</p>
            </div>
            <div>
              <h4 style={{ color: '#fff', fontWeight: 900, marginBottom: '20px', fontSize: '18px', textTransform: 'uppercase' }}>Circuit Contact</h4>
              <p style={{ fontSize: '14px', color: '#a1a1aa', marginBottom: '10px', fontWeight: 600 }}>✉️ {config.contactEmail}</p>
              <p style={{ fontSize: '14px', color: '#a1a1aa', fontWeight: 600 }}>📞 {config.contactPhone}</p>
            </div>
          </div>
          <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <p style={{ fontSize: '13px', color: '#52525b', fontWeight: 600 }}>© 2026 {config.title}. All rights reserved.</p>
            <div onClick={() => setCurrentView('admin')} style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#52525b', fontSize: '12px', cursor: 'pointer', fontWeight: 900, transition: '0.3s', letterSpacing: '1px' }} onMouseOver={e => e.currentTarget.style.color='var(--primary)'} onMouseOut={e => e.currentTarget.style.color='#52525b'}>
        ...
        {selectedSpeaker && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', backdropFilter: 'blur(10px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 2000, padding: '20px' }} onClick={() => setSelectedSpeaker(null)}>
            <div className="glass" style={{ borderRadius: '40px', width: '100%', maxWidth: '1000px', display: 'flex', overflow: 'hidden', position: 'relative' }} onClick={e => e.stopPropagation()}>
              <button onClick={() => setSelectedSpeaker(null)} style={{ position:'absolute', top:'25px', right:'25px', background:'rgba(255,255,255,0.1)', border:'none', width: '45px', height: '45px', borderRadius: '50%', color:'#fff', cursor:'pointer', zIndex:10, fontWeight: 'bold', fontSize: '18px' }}>✕</button>
              <div style={{ flex: '1', minHeight: '500px' }}><img src={selectedSpeaker.img} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(50%) contrast(1.2)' }} alt="speaker" /></div>
              <div style={{ flex: '1.2', padding: '60px' }}>
                <span style={{ display: 'inline-block', background: selectedSpeaker.color, color: '#fff', padding: '8px 24px', fontSize: '12px', fontWeight: 900, letterSpacing: '2px', marginBottom: '25px', borderRadius: '50px' }}>{selectedSpeaker.tag}</span>
                <h3 style={{ fontSize: '42px', fontWeight: 900, margin: '0 0 10px', color: '#fff', letterSpacing: '-1px', textTransform: 'uppercase' }}>{selectedSpeaker.name}</h3>
                <div style={{ fontSize: '18px', color: 'var(--primary)', fontWeight: '800', marginBottom: '30px', letterSpacing: '1px', textTransform: 'uppercase' }}>{selectedSpeaker.role}</div>
                <div style={{ width: '60px', height: '4px', background: 'var(--primary)', marginBottom: '30px', borderRadius: '50px' }}></div>
                <p style={{ color: '#d4d4d8', lineHeight: '1.8', fontSize: '16px', fontWeight: 500 }}>{selectedSpeaker.desc}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {ticketModal.isOpen && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', backdropFilter: 'blur(15px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 2000, padding: '20px' }}>
            <div className="glow-box" style={{ background: '#18181b', borderRadius: '40px', border: '2px solid var(--primary)', width: '100%', maxWidth: '450px', padding: '50px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
              <div style={{ width: '80px', height: '80px', background: 'var(--primary)', color: '#fff', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '40px', margin: '0 auto 20px', fontWeight: 900 }}>✓</div>
              <h2 style={{ marginBottom: '10px', color: '#fff', fontSize: '28px', fontWeight: 900, textTransform: 'uppercase' }}>Registration Success!</h2>
              <p style={{ color: '#a1a1aa', fontSize: '15px', marginBottom: '30px', fontWeight: 600 }}>ระบบส่ง E-TICKET และ PADDOCK PASS ไปยังอีเมลแล้ว</p>
              
              <div style={{ background: '#fff', padding: '20px', display: 'inline-block', margin: '0 auto 30px', borderRadius: '24px', boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }}><img src={ticketModal.qrUrl} alt="QR" width="180" style={{ display: 'block' }} /></div>
              
              <div style={{ background: '#0f0f11', padding: '20px', marginBottom: '30px', border: '1px solid var(--border)', textAlign: 'left', borderRadius: '20px' }}>
                <div>
              ...
              <button className="btn btn-primary" style={{ width: '100%', padding: '20px', fontSize: '18px' }} onClick={() => setTicketModal({ isOpen: false, name: '', tier: '', qrUrl: '' })}>CLOSE WINDOW</button>
            </div>
          </div>
        )}
      </>
  ...
  // ==========================================
  // RENDER: ADMIN VIEW (ROUNDED TELEMETRY DASHBOARD)
  // ==========================================
  return (
    <>
      <script src="https://cdn.tailwindcss.com"></script>
      
      <div className="flex h-screen bg-[#050505] text-slate-200 font-sans overflow-hidden selection:bg-red-500/30">
        
        {/* Sidebar */}
        <aside className="w-72 bg-[#0a0a0a] border-r border-white/5 flex flex-col z-10 shadow-2xl relative">
          <div className="absolute top-0 left-0 w-full h-1" style={{ background: 'linear-gradient(90deg, var(--primary), var(--secondary))' }}></div>
          <div className="p-8 border-b border-white/5 flex items-center gap-4 cursor-pointer hover:bg-white/5 transition-colors" onClick={() => setCurrentView('customer')}>
          ...
          <nav className="flex-1 p-6 space-y-2 overflow-y-auto">
            ...
            <button onClick={() => setAdminTab('dashboard')} className={`w-full flex items-center gap-4 px-4 py-3.5 text-sm font-bold transition-all rounded-2xl ${adminTab === 'dashboard' ? 'bg-white/10 text-white shadow-lg' : 'text-zinc-400 hover:bg-white/5 hover:text-zinc-200'}`}>
              <span className="text-xl">📊</span> Live Timing
            </button>
            <button onClick={() => setAdminTab('users')} className={`w-full flex items-center gap-4 px-4 py-3.5 text-sm font-bold transition-all rounded-2xl ${adminTab === 'users' ? 'bg-white/10 text-white shadow-lg' : 'text-zinc-400 hover:bg-white/5 hover:text-zinc-200'}`}>
            ...
            <button onClick={() => setAdminTab('settings')} className={`w-full flex items-center gap-4 px-4 py-3.5 text-sm font-bold transition-all rounded-2xl ${adminTab === 'settings' ? 'bg-white/10 text-white shadow-lg' : 'text-zinc-400 hover:bg-white/5 hover:text-zinc-200'}`}>
              <span className="text-xl">⚙️</span> Track Config
            </button>
            <button onClick={() => setAdminTab('schedule')} className={`w-full flex items-center gap-4 px-4 py-3.5 text-sm font-bold transition-all rounded-2xl ${adminTab === 'schedule' ? 'bg-white/10 text-white shadow-lg' : 'text-zinc-400 hover:bg-white/5 hover:text-zinc-200'}`}>
              <span className="text-xl">⏱️</span> Schedule
            </button>
            <button onClick={() => setAdminTab('speakers')} className={`w-full flex items-center gap-4 px-4 py-3.5 text-sm font-bold transition-all rounded-2xl ${adminTab === 'speakers' ? 'bg-white/10 text-white shadow-lg' : 'text-zinc-400 hover:bg-white/5 hover:text-zinc-200'}`}>
              <span className="text-xl">🏍️</span> Riders
            </button>
            <button onClick={() => setAdminTab('tickets')} className={`w-full flex items-center gap-4 px-4 py-3.5 text-sm font-bold transition-all rounded-2xl ${adminTab === 'tickets' ? 'bg-white/10 text-white shadow-lg' : 'text-zinc-400 hover:bg-white/5 hover:text-zinc-200'}`}>
        ...
        {/* Main Content Area */}
        <main className="flex-1 p-8 lg:p-12 overflow-y-auto relative" style={{ backgroundColor: '#050505', backgroundImage: 'radial-gradient(rgba(255,255,255,0.03) 1px, transparent 1px)', backgroundSize: '24px 24px' }}>
          ...
          {adminTab === 'dashboard' && (
            <div className="max-w-7xl mx-auto space-y-8 relative z-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-[#0a0a0a] border border-white/5 p-8 rounded-3xl relative overflow-hidden group hover:border-red-500/50 transition-colors">
                  <div className="w-12 h-12 bg-red-500/10 text-red-500 flex items-center justify-center rounded-2xl text-2xl mb-6">👥</div>
                  <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2">Total Entries</p>
                  <div className="text-5xl font-black text-white">{registrations.length}</div>
                </div>
                <div className="bg-[#0a0a0a] border border-white/5 p-8 rounded-3xl relative overflow-hidden group hover:border-emerald-500/50 transition-colors">
                  <div className="w-12 h-12 bg-emerald-500/10 text-emerald-500 flex items-center justify-center rounded-2xl text-2xl mb-6">💰</div>
                  <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2">Net Revenue</p>
                  <div className="text-4xl font-black text-white font-mono">฿{totalRevenueNum.toLocaleString()}</div>
                </div>
                <div className="bg-[#0a0a0a] border border-white/5 p-8 rounded-3xl relative overflow-hidden group hover:border-yellow-500/50 transition-colors">
                  <div className="w-12 h-12 bg-yellow-500/10 text-yellow-500 flex items-center justify-center rounded-2xl text-2xl mb-6">🏆</div>
                  <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2">Top Category</p>
                  <div className="text-2xl font-black text-white mt-2 uppercase">{topTicket?.name || '-'}</div>
                </div>
                <div className="bg-[#0a0a0a] border border-white/5 p-8 rounded-3xl relative overflow-hidden group hover:border-blue-500/50 transition-colors">
                  <div className="w-12 h-12 bg-blue-500/10 text-blue-500 flex items-center justify-center rounded-2xl text-2xl mb-6">📊</div>
                  <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2">Avg. Value</p>
                  <div className="text-4xl font-black text-white font-mono">฿{avgOrderValue.toLocaleString()}</div>
                </div>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-1 bg-[#0a0a0a] p-8 border border-white/5 rounded-3xl">
                  <h3 className="text-lg font-black text-white mb-8 uppercase">Pass Allocation</h3>
                  <div className="space-y-6">
                    {ticketStats.map((t, i) => (
                      <div key={i}>
                        <div className="flex justify-between text-xs font-bold mb-3 uppercase">
                          <span className="text-zinc-400">{t.name}</span>
                          <span className="text-white">{t.count} <span className="text-zinc-600 font-normal font-mono">({t.percent}%)</span></span>
                        </div>
                ...
                <div className="lg:col-span-2 bg-[#0a0a0a] p-8 border border-white/5 flex flex-col rounded-3xl">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-lg font-black text-white uppercase">Recent Entries</h3>
                    <button onClick={() => setAdminTab('users')} className="text-[10px] font-black text-zinc-500 hover:text-white uppercase tracking-widest bg-white/5 px-4 py-2 rounded-full transition-colors">View All →</button>
                  </div>
                  {registrations.length === 0 ? (
                    <div className="flex-1 flex flex-col items-center justify-center text-zinc-600 font-mono text-sm py-12 border border-dashed border-white/10 rounded-2xl">
                      NO DATA TRANSMITTED
                    </div>
                  ) : (
                    <div className="space-y-3 flex-1">
                      {registrations.slice(0, 5).map(r => (
                        <div key={r.id} className="flex items-center justify-between p-4 bg-white/5 border border-white/5 hover:border-white/10 transition-colors rounded-2xl">
                          <div className="flex items-center gap-4">
                            <div className="font-mono text-zinc-600 text-xs w-16 truncate">{r.id.toString().slice(-6)}</div>
                            <div>
                              <div className="font-bold text-white text-sm uppercase">{r.name}</div>
                              <div className="text-[10px] text-zinc-500 font-mono uppercase tracking-wider">{r.company || r.email}</div>
                            </div>
                          </div>
          ...
          {adminTab === 'users' && (
             <div className="max-w-7xl mx-auto space-y-6 relative z-10">
               <h2 className="text-3xl font-black text-white tracking-tight mb-6 uppercase">Entry List</h2>
               <div className="bg-[#0a0a0a] border border-white/5 rounded-3xl overflow-hidden">
                 {registrations.length === 0 ? (
                   <div className="text-center py-20 text-zinc-600 text-sm font-mono uppercase">Awaiting Entries...</div>
                 ) : (
                   <div className="overflow-x-auto">
                     <table className="w-full text-left border-collapse">
                       <thead>
                         <tr className="bg-white/5 border-b border-white/5 text-[10px] font-black text-zinc-500 uppercase tracking-widest">
                           <th className="py-5 px-8">Rider / Crew Info</th>
                           <th className="py-5 px-8">Comms</th>
                           <th className="py-5 px-8">Pass Type</th>
                           <th className="py-5 px-8 font-mono text-right">Fee (THB)</th>
                       ...
                       <tbody className="divide-y divide-white/5 text-sm">
                         {registrations.map(r => (
                           <tr key={r.id} className="hover:bg-white/[0.02] transition-colors">
                             {editingUserId === r.id ? (
                               <>
                                 <td className="py-4 px-8 space-y-2">
                                   <input className="w-full p-3 bg-black border border-white/10 text-sm text-white font-bold uppercase rounded-xl focus:border-red-500 outline-none" value={editUserForm.name} onChange={e => setEditUserForm({...editUserForm, name: e.target.value})} placeholder="Name" />
                                   <input className="w-full p-3 bg-black border border-white/10 text-xs text-white uppercase rounded-xl focus:border-red-500 outline-none" value={editUserForm.company} onChange={e => setEditUserForm({...editUserForm, company: e.target.value})} placeholder="Team" />
                                 </td>
                                 <td className="py-4 px-8 space-y-2">
                                 ...
                                 <td className="py-4 px-8 text-center space-x-2 whitespace-nowrap">
                                   ...
                                   <button onClick={() => setEditingUserId(null)} className="px-4 py-2 bg-white/5 hover:bg-white/10 text-white text-[10px] font-black uppercase tracking-wider border border-white/10 rounded-full transition-colors">Cancel</button>
                                 </td>
                               </>
                             ) : (
                               <>
                                 <td className="py-6 px-8">
                                   ...
                                   <div className="text-[10px] text-zinc-500 font-bold mt-1 uppercase tracking-wider">{r.company || '-'}</div>
                                 </td>
                                 <td className="py-6 px-8">
                                   <div className="text-zinc-300 font-mono text-xs">{r.email}</div>
                                 ...
                                 <td className="py-6 px-8 text-center space-x-2 whitespace-nowrap">
                                   <button onClick={() => startEditUser(r)} className="px-4 py-1.5 text-zinc-400 hover:text-white bg-black border border-white/10 font-black text-[10px] uppercase tracking-wider rounded-full transition-colors" title="Edit">Edit</button>
                                   <button onClick={() => deleteUser(r.id)} className="px-4 py-1.5 text-rose-500 hover:text-white bg-black border border-white/10 font-black text-[10px] uppercase tracking-wider rounded-full transition-colors" title="Delete">Del</button>
                                 </td>
                               </>
                             )}
                           </tr>
           ...
           {adminTab === 'settings' && (
            <div className="max-w-5xl mx-auto space-y-6 relative z-10">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <h2 className="text-3xl font-black text-white tracking-tight uppercase">Event Configuration</h2>
                <button onClick={handleSaveConfig} className="px-8 py-3.5 bg-emerald-500 hover:bg-emerald-400 text-black font-black uppercase text-xs tracking-widest shadow-lg shadow-emerald-500/20 transition-all rounded-full">
                  💾 SAVE CONFIG
                </button>
              </div>
              ...
              <div className="bg-[#0a0a0a] p-8 border border-white/5 shadow-xl space-y-6 rounded-3xl">
                <div className="border-b border-white/5 pb-4">
                  ...
                  <p className="text-[10px] text-zinc-500 uppercase tracking-widest mt-1">ใส่ลิงก์รูปภาพ (URL) สำหรับเปลี่ยนพื้นหลังเว็บ</p>
                </div>
                <div className="grid grid-cols-1 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Hero Background (ภาพเปิดด้านบนสุด)</label>
                    <input type="text" value={config.heroBg} onChange={(e) => setConfig({...config, heroBg: e.target.value})} placeholder="https://..." className="w-full p-4 bg-black border border-white/10 text-sm font-mono text-blue-400 focus:border-red-500 outline-none rounded-xl" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Sponsors Background (ภาพพื้นหลังแบรนด์สปอนเซอร์)</label>
                    <input type="text" value={config.sponsorBg} onChange={(e) => setConfig({...config, sponsorBg: e.target.value})} placeholder="https://..." className="w-full p-4 bg-black border border-white/10 text-sm font-mono text-blue-400 focus:border-red-500 outline-none rounded-xl" />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#0a0a0a] p-8 border border-white/5 shadow-xl space-y-6 rounded-3xl">
                <div className="flex justify-between items-center border-b border-white/5 pb-4">
                  <h3 className="text-lg font-black text-white uppercase">🎬 VOD Highlight</h3>
                  <label className="flex items-center cursor-pointer">
                    <div className="relative">
                      ...
                      <div className={`block w-12 h-6 transition-colors border border-white/10 rounded-full ${config.showVideo ? 'bg-emerald-500' : 'bg-black'}`}></div>
                      <div className={`dot absolute left-1 top-1 w-4 h-4 rounded-full transition-transform ${config.showVideo ? 'bg-black transform translate-x-6' : 'bg-zinc-600'}`}></div>
                    </div>
                    <span className="ml-3 text-[10px] font-black text-zinc-500 uppercase tracking-widest">{config.showVideo ? 'LIVE' : 'OFFLINE'}</span>
                  </label>
                </div>
                ...
                {config.showVideo && (
                  <div className="grid grid-cols-1 gap-6">
                    <div className="space-y-2">
                      ...
                      <input type="text" value={config.videoTitle} onChange={(e) => setConfig({...config, videoTitle: e.target.value})} className="w-full p-4 bg-black border border-white/10 text-sm font-bold text-white focus:border-red-500 outline-none uppercase rounded-xl" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">YouTube Video Link</label>
                      <input type="text" value={config.videoUrl} onChange={(e) => setConfig({...config, videoUrl: e.target.value})} className="w-full p-4 bg-black border border-white/10 text-sm text-blue-400 font-mono focus:border-red-500 outline-none rounded-xl" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Description</label>
                      <textarea value={config.videoDesc} onChange={(e) => setConfig({...config, videoDesc: e.target.value})} className="w-full p-4 bg-black border border-white/10 text-xs text-zinc-300 focus:border-red-500 outline-none rounded-xl" rows="3"></textarea>
                    </div>
                  </div>
                )}
              </div>

              <div className="bg-[#0a0a0a] p-8 border border-white/5 shadow-xl space-y-8 rounded-3xl">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-white/5 border border-white/10 rounded-2xl">
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Primary Color (Brand)</label>
                        <div className="flex gap-3">
                            <input type="color" value={config.primaryColor} onChange={(e) => setConfig({...config, primaryColor: e.target.value})} className="w-14 h-14 bg-black border border-white/10 cursor-pointer p-1 rounded-xl" />
                            <input type="text" value={config.primaryColor} onChange={(e) => setConfig({...config, primaryColor: e.target.value})} className="flex-1 p-4 bg-black border border-white/10 text-sm font-bold text-white font-mono uppercase outline-none rounded-xl" />
                        </div>
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Secondary Color</label>
                        <div className="flex gap-3">
                            <input type="color" value={config.secondaryColor} onChange={(e) => setConfig({...config, secondaryColor: e.target.value})} className="w-14 h-14 bg-black border border-white/10 cursor-pointer p-1 rounded-xl" />
                            <input type="text" value={config.secondaryColor} onChange={(e) => setConfig({...config, secondaryColor: e.target.value})} className="flex-1 p-4 bg-black border border-white/10 text-sm font-bold text-white font-mono uppercase outline-none rounded-xl" />
                        </div>
                    </div>
                </div>
          ...
          {adminTab === 'speakers' && (
            <div className="max-w-6xl mx-auto space-y-6 relative z-10">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <h2 className="text-3xl font-black text-white tracking-tight uppercase">Riders & Crew</h2>
                <button onClick={handleSaveConfig} className="px-8 py-3.5 bg-emerald-500 hover:bg-emerald-400 text-black font-black uppercase text-xs tracking-widest shadow-lg rounded-full transition-all">
                  💾 SAVE CONFIG
                </button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {config.speakers?.map(speaker => (
                  <div key={speaker.id} className="bg-[#0a0a0a] p-8 border border-white/5 relative rounded-3xl shadow-xl">
                    <button onClick={() => removeArrayItem('speakers', speaker.id)} className="absolute top-6 right-6 px-4 py-2 bg-rose-500/10 text-rose-500 border border-rose-500/20 text-[10px] font-black uppercase hover:bg-rose-500/20 transition-all rounded-full">Delete</button>
                    <div className="flex flex-col xl:flex-row gap-6 items-start mb-6">
                      <img src={speaker.img} className="w-28 h-28 object-cover border-2 border-white/10 rounded-2xl" alt="speaker" />
                      
                      <div className="flex-1 w-full space-y-3">
                        <div className="space-y-1.5">
                          <label className="block text-[10px] font-black text-zinc-500 uppercase tracking-widest">Image URL (วางลิงก์รูป)</label>
                          <input type="text" value={speaker.img} onChange={(e) => handleArrayChange('speakers', speaker.id, 'img', e.target.value)} className="w-full p-3 bg-black border border-white/10 text-xs font-mono text-blue-400 focus:border-red-500 outline-none rounded-xl" placeholder="https://..." />
                        </div>
                        <div className="space-y-1.5">
                          <label className="block text-[10px] font-black text-zinc-500 uppercase tracking-widest">Or Upload File</label>
                          <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e, 'speakers', speaker.id)} className="w-full text-xs text-zinc-400 file:mr-4 file:py-2 file:px-4 file:border-0 file:text-[10px] file:font-black file:uppercase file:bg-white/5 file:text-white hover:file:bg-white/10 file:rounded-full cursor-pointer" />
                        </div>
                      </div>
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <input type="text" value={speaker.name} onChange={(e) => handleArrayChange('speakers', speaker.id, 'name', e.target.value)} className="w-full p-3.5 bg-black border border-white/10 text-sm font-black uppercase text-white focus:border-red-500 outline-none rounded-xl" placeholder="Name" />
                        <input type="text" value={speaker.role} onChange={(e) => handleArrayChange('speakers', speaker.id, 'role', e.target.value)} className="w-full p-3.5 bg-black border border-white/10 text-xs font-bold uppercase text-zinc-400 focus:border-red-500 outline-none rounded-xl" placeholder="Team" />
                      </div>
                      <div className="flex gap-4">
                         <input type="text" value={speaker.tag} onChange={(e) => handleArrayChange('speakers', speaker.id, 'tag', e.target.value)} className="flex-1 p-3.5 bg-black border border-white/10 text-xs font-black text-white uppercase focus:border-red-500 outline-none rounded-xl" placeholder="Category" />
                         <input type="color" value={speaker.color} onChange={(e) => handleArrayChange('speakers', speaker.id, 'color', e.target.value)} className="w-14 h-12 bg-black border border-white/10 cursor-pointer p-1 rounded-xl" />
                      </div>
                      <textarea value={speaker.desc} onChange={(e) => handleArrayChange('speakers', speaker.id, 'desc', e.target.value)} className="w-full p-4 bg-black border border-white/10 text-sm text-zinc-400 focus:border-red-500 outline-none rounded-xl" rows="3" placeholder="Bio..."></textarea>
                    </div>
                  </div>
                ))}
                <button onClick={addSpeaker} className="min-h-[300px] border-2 border-dashed border-white/10 hover:border-white/30 text-zinc-500 hover:text-white font-black text-sm uppercase transition-all flex flex-col items-center justify-center gap-4 bg-[#0a0a0a] rounded-3xl">
                  <span className="text-5xl font-light">+</span> Add Personnel
                </button>
              </div>
            </div>
          )}
          ...
          {adminTab === 'schedule' && (
            <div className="max-w-4xl mx-auto space-y-6 relative z-10">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <h2 className="text-3xl font-black text-white tracking-tight uppercase">Race Schedule</h2>
                <button onClick={handleSaveConfig} className="px-8 py-3.5 bg-emerald-500 hover:bg-emerald-400 text-black font-black uppercase text-xs tracking-widest shadow-lg rounded-full transition-all">
                  💾 SAVE CONFIG
                </button>
              </div>
              <div className="space-y-6">
                {config.schedule?.map(s => (
                  <div key={s.id} className="bg-[#0a0a0a] p-8 border border-white/5 flex flex-col md:flex-row gap-8 relative group rounded-3xl shadow-xl">
                    <button onClick={() => removeArrayItem('schedule', s.id)} className="absolute top-6 right-6 opacity-0 group-hover:opacity-100 px-4 py-2 bg-rose-500/10 text-rose-500 border border-rose-500/20 text-[10px] font-black uppercase transition-all rounded-full">Delete</button>
                    <div className="w-full md:w-32 space-y-4">
                      <input type="text" value={s.time} onChange={(e) => handleArrayChange('schedule', s.id, 'time', e.target.value)} className="w-full p-4 bg-black border border-white/10 text-center font-black text-2xl text-white focus:border-red-500 outline-none rounded-xl" />
                      <input type="color" value={s.color} onChange={(e) => handleArrayChange('schedule', s.id, 'color', e.target.value)} className="w-full h-10 bg-black border border-white/10 cursor-pointer p-1 rounded-xl" />
                    </div>
                    <div className="flex-1 space-y-4 pr-10">
                      <div className="flex gap-4">
                        <input type="text" value={s.tag} onChange={(e) => handleArrayChange('schedule', s.id, 'tag', e.target.value)} className="w-32 p-3 bg-black border border-white/10 text-[10px] font-black text-zinc-400 uppercase tracking-widest focus:border-red-500 outline-none rounded-xl" placeholder="TAG" />
                        <input type="text" value={s.title} onChange={(e) => handleArrayChange('schedule', s.id, 'title', e.target.value)} className="flex-1 p-3 bg-black border border-white/10 text-sm font-black uppercase text-white focus:border-red-500 outline-none rounded-xl" placeholder="Session Title" />
                      </div>
                      <textarea value={s.desc} onChange={(e) => handleArrayChange('schedule', s.id, 'desc', e.target.value)} className="w-full p-4 bg-black border border-white/10 text-sm text-zinc-400 focus:border-red-500 outline-none rounded-xl" rows="3" placeholder="Description"></textarea>
                    </div>
                  </div>
                ))}
                <button onClick={addSchedule} className="w-full py-8 border-2 border-dashed border-white/10 hover:border-white/30 text-zinc-500 hover:text-white font-black text-sm uppercase transition-all bg-[#0a0a0a] rounded-3xl">
                  + Add Track Session
                </button>
              </div>
            </div>
          ...
          {adminTab === 'tickets' && (
            <div className="max-w-6xl mx-auto space-y-6 relative z-10">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                 <h2 className="text-3xl font-black text-white tracking-tight uppercase">Ticketing & Passes</h2>
                 <div className="flex gap-4">
                   <button onClick={addTicket} className="px-6 py-3.5 bg-white/10 hover:bg-white/20 text-white text-[11px] font-black uppercase tracking-wider shadow-lg border border-white/10 transition-all rounded-full">+ Add Pass</button>
                   <button onClick={handleSaveConfig} className="px-8 py-3.5 bg-emerald-500 hover:bg-emerald-400 text-black font-black uppercase text-xs tracking-widest shadow-lg rounded-full transition-all">
                     💾 SAVE CONFIG
                   </button>
                 </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {config.tickets?.map(ticket => (
                  <div key={ticket.id} className="bg-[#0a0a0a] p-8 border border-white/5 flex flex-col gap-5 relative rounded-3xl shadow-xl">
                    <button onClick={() => removeArrayItem('tickets', ticket.id)} className="absolute top-6 right-6 px-4 py-2 bg-rose-500/10 text-rose-500 border border-rose-500/20 text-[10px] font-black uppercase transition-all z-10 rounded-full">Delete</button>
                    
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mt-2">Pass Name</label>
                      <input type="text" value={ticket.name} onChange={(e) => handleArrayChange('tickets', ticket.id, 'name', e.target.value)} className="w-full p-4 bg-black border border-white/10 text-lg font-black uppercase text-white focus:border-red-500 outline-none text-center rounded-xl" />
                    </div>

                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mt-2">Price (THB)</label>
                      <div className="relative">
                         <span className="absolute left-5 top-4 text-zinc-600 font-black">฿</span>
                         <input type="number" value={ticket.price} onChange={(e) => handleArrayChange('tickets', ticket.id, 'price', e.target.value)} className="w-full p-4 pl-10 bg-black border border-white/10 text-2xl font-black font-mono text-emerald-400 focus:border-red-500 outline-none text-center rounded-xl" />
                      </div>
                    </div>

                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mt-2">Highlight Badge (Optional)</label>
                      <input type="text" value={ticket.badge || ''} onChange={(e) => handleArrayChange('tickets', ticket.id, 'badge', e.target.value)} className="w-full p-3 bg-black border border-white/10 text-xs font-bold text-yellow-500 uppercase text-center outline-none rounded-xl" placeholder="e.g. VIP ZONE" />
                    </div>
